...
public interface ListaRepository extends JpaRepository<Lista, Integer> {
    @Modifying
    @Transactional
    @Query("UPDATE Lista l SET l.lista_status = -1 WHERE l.id = :id")
    void apagadoLogicoLista(@Param("id") Integer item);

    @Query("SELECT l from Lista l WHERE l.lista_status >= 0")
    List<Lista> listarListas();

    @Query("SELECT l from Lista l where l.id=:id AND l.lista_status >=0")
    Lista obterListaPeloId(@Param("id") Integer listaId);


    @Query("SELECT l FROM Lista l WHERE l.usuario.id = :usuario_id")
